...
export async function POST(_request: Request): Promise<Response> {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to authorize a charge: ${response.statusText} (${response.status})`
      );
    }
    ...
    // Capture the charge.
    const captureUrl = `${SMALL_TRANSFERS_API_BASE_URL}/charges/${chargeExternalId}/capture`;
    ...
    if (!captureResponse.ok) {
      throw new Error(
        `Failed to capture a charge: ${captureResponse.statusText} (${captureResponse.status})`
      );
    }
